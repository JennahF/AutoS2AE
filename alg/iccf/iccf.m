...
[X, Y, alpha, test, max_iter, K, P, Q, reg_u, reg_i, is_item_fixed, is_user_fixed, ...
    user_bias, item_bias, usr_w, item_w, k_verbose, R_v] = ...
   process_options(varargin, 'X', zeros(M,0), 'Y', zeros(N,0), 'alpha', 30, 'test', [], 'max_iter', 20, 'K', 64, 'P', [], 'Q', [], ...
   'reg_u', 0.01, 'reg_i', 0.01, 'is_item_fixed', false, 'is_user_fixed', false, 'user_bias', false, 'item_bias', false,...
   'usr_w', ones(M,1), 'item_w', ones(N,1), 'k-v', 5, 'valid', 0);
...
for iter=1:max_iter
    ...
    if mod(iter, k_verbose) ==0 && (~isempty(test))
        metric = evaluate_item(R+R_v, test, P, Q, 200, 200);
        if isexplict(test)
